...
public class Insere {
     ...
     */
    public boolean insereLomtoObaa(HashMap objetos, String dnRecebido, LDAPConnection lc, Indexador indexar, int idRep) {
        ...
        while (iter.hasNext()) {
            ...
            if (!atributoLom.contains("obaaIdentifierRaiz") && !objetos.get(atributoLom).toString().isEmpty()) {
                ...
                if (!atributoObaa.contains("obaaCaracUsuario") && !atributoObaa.contains("obaaCaracDesc") && !atributoObaa.contains("obaaCaracAmbiente") && !atributoObaa.contains("obaaContEnt") && !atributoObaa.contains("obaaCaracTipo") && !atributoObaa.contains("obaaCaracFaixa")) {
                    ...
                    else if (atributoObaa.equalsIgnoreCase("obaaDate")) {
                        for (int i = 0; i < valorTemporario.length; i++) {
                            doc.setData(valorTemporario[i]);
                        }
                    }//fim if Date
                    else if (atributoObaa.equalsIgnoreCase("obaaLocation")) {
                        for (int i = 0; i < valorTemporario.length; i++) {
                            doc.setLocalizacao(valorTemporario[i]);
                        }
                    }//fim if location
                    else if (atributoObaa.equalsIgnoreCase("obaaResourceEntry")) {
                        for (int i = 0; i < valorTemporario.length; i++) {
                            doc.setLocalizacao(valorTemporario[i]); //ver depois se nao eh melhor colocar um setResourceEntry   
                        }
                    }//fim if location
