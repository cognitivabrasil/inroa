...
public class Indexador {
	...
	public void addDoc(Documento doc, Connection con) throws SQLException {
        ...
        String sql = "INSERT INTO documentos (obaaEntry) VALUES ('" + doc.getObaaEntry() + "');";
        int key = 0;
	...
	public void populateR1(Connection con) throws SQLException {
		...
		PreparedStatement R1Size = con
				.prepareStatement("INSERT INTO r1size(size) SELECT COUNT(DISTINCT(id)) FROM r1tokens;");

		R1Size.execute();
		R1Size.close();

		PreparedStatement R1IDF = con
				.prepareStatement("INSERT INTO r1idf(token, idf) SELECT T.token, LOG(S.size)-LOG(COUNT(DISTINCT(id))) FROM r1tokens T, r1size S group by T.token, S.size;");
		R1IDF.execute();
		R1IDF.close();
		
		PreparedStatement R1TF = con
				.prepareStatement("INSERT INTO r1tf(tid, token, tf) SELECT T.id, T.token, if(T.field=1||T.field=2,COUNT(*)*2, COUNT(*)) FROM r1tokens T GROUP BY T.id, T.token;");
		R1TF.execute();
		R1TF.close();

		PreparedStatement R1Lenght = con
				.prepareStatement("INSERT INTO r1length(tid, len) SELECT T.tid, SQRT(SUM(I.idf*I.idf*T.tf*T.tf)) FROM r1idf I, r1tf T WHERE I.token = T.token GROUP BY T.tid;");
		R1Lenght.execute();
		R1Lenght.close();

		PreparedStatement R1Weights = con
				.prepareStatement("INSERT INTO r1weights(tid, token, weight) SELECT T.tid, T.token, I.idf*T.tf/L.len FROM r1idf I, r1tf T, r1length L WHERE I.token = T.token AND T.tid = L.tid;");
		R1Weights.execute();
		R1Weights.close();

		PreparedStatement R1Sum = con
				.prepareStatement("INSERT INTO r1sum(token, total) SELECT R.token, SUM(R.weight) FROM r1weights R GROUP BY R.token;");
		R1Sum.execute();
		R1Sum.close();
