...
public class MapeamentoValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if(isBlank(m.getName())) {
			...
			errors.rejectValue("name", "description.exists", "O nome já existe.");
		}
		
		if(isBlank(m.getDescription())) {
			errors.rejectValue("description", "description.empty", "Informe uma descrição");
		}
		...
		if(padraoDao.get(m.getPadraoMetadados()) == null) {
			errors.rejectValue("padraoMetadados", "padraoMetadados.empty", "Informe uma padrão de metadados");
		}
