...
public class Repositorio implements java.io.Serializable, SubNodo {
	...
	public Integer getSize() {
		return DataAccessUtils
				.intResult(getSession()
						.createQuery(
								"select count(*) from DocumentoReal doc WHERE doc.repositorio = :rep AND doc.deleted = :deleted")
						.setParameter("rep", this)
	...
	public List<Repositorio> getOutDatedDocs() {
		...
		return getSession()
				.createSQLQuery(
						"SELECT * FROM repositorios r WHERE r.data_ultima_atualizacao <= ((now() - r.periodicidade_horas * INTERVAL '1 DAY') + INTERVAL '4 HOUR')")
				.addEntity(Repositorio.class).list();
	}
	...
	public Date getProximaAtualizacao() {
		if (ultimaAtualizacao == null) { // TODO: Jorjao porque ta dando null se
											// no construtor ta inicializando?
			return new Date(0);
		}
	...
	public Set<String> getColecoes() {
		if (colecoesInternal.isEmpty()) {
			...
			return new HashSet<String>(Arrays.asList(colecoesInternal
					.split(";")));
		}
	}
	...
	/**
	 * @param padraoMetadados
	 *            the padraoMetadados to set
	 */
	 ...
	 * 
	 * @param r2
	 *            A repository that we want to update.
