...
public class Excluir {
     ...
     * @param id id do reposit&oacute;rio ou subreposit&oacute;rio do documento
     * @param con Conex&atilde;o com a base de dados.
     * @param ondeRemover String "rep" ou "subrep" informando se deve remover o documento do reposit&oacute;rio ou do reposit&oacute;rio da subfedera&ccedil;&atilde;o respectivamente.
     * @return true se a exclus&atilde;o foi realizada com sucesso e false se n&atilde;o foi possível excluir
     * @throws SQLException
     */
    public static boolean removerDocumentoIndice(String obaa_entry, int id, Connection con, String ondeRemover) throws SQLException {

        int result = 0;

        Statement stm = con.createStatement();
        String sql = "";
        if (ondeRemover.equalsIgnoreCase("subRep")) {
            sql = "DELETE FROM documentos where obaa_entry='" + obaa_entry + "' AND id_rep_subfed =" + id;
        } else {
            sql = "DELETE FROM documentos where obaa_entry='" + obaa_entry + "' AND id_repositorio =" + id;
        }
        //testar conexao
        try {
