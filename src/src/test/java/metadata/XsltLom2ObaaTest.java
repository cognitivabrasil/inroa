...
public class XsltLom2ObaaTest {
 ...
 "net.sf.saxon.TransformerFactoryImpl");
	...
	@Before
	public void setUp() {
 		String foo_xml = "src/test/java/metadata/oai_lom.xml"; //input xml
	...
	public void testDC2OBAA_identifier() {
		...
		assertThat(l.getGeneral().getIdentifier().getEntry(), equalTo("baskjikM"));
	}
	...
	public void testDC2OBAA_General() {
		OBAA l = oai.getMetadata(0);
		assert(!(l.getGeneral() == null));
		assertThat(l.getGeneral().getDescriptions(), hasItems("Animação ilustrando a troca de mensagens que ocorre quando um intruso bloqueia um computador medianteo envio de mensagens de hadshake repetidas e depois impersona esta máquina enquanto ela está imobilizada."));
	...
	public void testCestaCompleteOutput() throws FileNotFoundException {
		OaiOBAA cesta;
 		String foo_xml = "src/test/java/metadata/cesta_complete.xml"; //input xml
		String foo_xsl = "src/xslt/lom2obaa_full.xsl"; //input xsl
